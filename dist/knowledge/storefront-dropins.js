...
export function findDropin(query) {
    const q = query.toLowerCase().trim();
    return DROPIN_CATALOG.find((d) => d.id === q ||
        d.blockName === q ||
        d.package === q ||
        d.title.toLowerCase().includes(q) ||
        d.id.includes(q));
}
export function listDropinsByCategory(category) {
    return DROPIN_CATALOG.filter((d) => d.category === category);
}
// Canonical mount snippet for any dropin
export function buildMountSnippet(spec, container = spec.containers[0]) {
...
//# sourceMappingURL=storefront-dropins.js.map