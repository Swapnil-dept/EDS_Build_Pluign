...
export function detectProjectType(input) {
    ...
    let score = 0;
    const add = (weight, source, detail) => {
    ...
    if (pkg) {
        ...
        if ('@dropins/tools' in deps)
            add(+30, 'package.json', '@dropins/tools is installed');
    ...
    for (const f of STOREFRONT_SCRIPTS) {
        if (scripts.includes(f))
            add(+6, 'scripts/', `${f} present`);
    }
    if (scripts.includes('aem.js'))
        add(-0, 'scripts/', 'aem.js present (EDS baseline)');
    // ─── root listing ──────────────────────────────────────────
    const root = listLines(input.rootDirListing);
    for (const f of STOREFRONT_FILES) {
        if (root.includes(f))
            add(+8, 'root', `${f} present`);
    }
    if (root.includes('postinstall.js'))
        add(+5, 'root', 'postinstall.js present');
    if (root.includes('component-models.json'))
        add(-0, 'root', 'component-models.json (UE — common to both)');
    // ─── blocks/ listing ───────────────────────────────────────
    ...
    if (cfg) {
        if (cfg['commerce-endpoint'])
            add(+15, 'config.json', 'commerce-endpoint set');
        if (cfg['commerce-environment-id'])
            add(+5, 'config.json', 'commerce-environment-id set');
        if (cfg['adobe-commerce-optimizer'])
            add(+5, 'config.json', 'adobe-commerce-optimizer = true');
        if (cfg.headers?.cs)
            add(+4, 'config.json', 'headers.cs commerce headers');
    }
    // ─── fstab.yaml ────────────────────────────────────────────
