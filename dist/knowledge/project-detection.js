...
export function detectProjectType(input) {
    const signals = [];
    const warnings = [];
    const installedDropins = [];
    let score = 0;
    let aemScore = 0;
    let aem65Score = 0;
    const addCommerce = (weight, source, detail) => {
        signals.push({ weight, source, detail });
        score += weight;
    };
    const addAem = (weight, source, detail) => {
        signals.push({ weight, bucket: 'aemaacs', source, detail });
        aemScore += weight;
    };
    const addAem65 = (weight, source, detail) => {
    ...
    if (pkg) {
        ...
        if ('@dropins/tools' in deps)
            addCommerce(+30, 'package.json', '@dropins/tools is installed');
    ...
    for (const f of STOREFRONT_SCRIPTS) {
        if (scripts.includes(f))
            addCommerce(+6, 'scripts/', `${f} present`);
    }
    // ─── root listing ──────────────────────────────────────────
    const root = listLines(input.rootDirListing);
    for (const f of STOREFRONT_FILES) {
        if (root.includes(f))
            addCommerce(+8, 'root', `${f} present`);
    }
    if (root.includes('postinstall.js'))
        addCommerce(+5, 'root', 'postinstall.js present');
    // ─── blocks/ listing ───────────────────────────────────────
    ...
    if (cfg) {
        if (cfg['commerce-endpoint'])
            addCommerce(+15, 'config.json', 'commerce-endpoint set');
        if (cfg['commerce-environment-id'])
            addCommerce(+5, 'config.json', 'commerce-environment-id set');
        if (cfg['adobe-commerce-optimizer'])
            addCommerce(+5, 'config.json', 'adobe-commerce-optimizer = true');
        if (cfg.headers?.cs)
            addCommerce(+4, 'config.json', 'headers.cs commerce headers');
    }
    ...
    if (listLines(input.uiAppsDirListing).length) {
        ...
        if (!detectedAemModules.includes('ui.apps'))
            detectedAemModules.push('ui.apps');
    }
    if (listLines(input.coreDirListing).length) {
        ...
        if (!detectedAemModules.includes('core'))
            detectedAemModules.push('core');
    }
    if (listLines(input.dispatcherDirListing).length) {
        ...
        if (!detectedAemModules.includes('dispatcher'))
            detectedAemModules.push('dispatcher');
    }
    ...
    if (installedDropins.length && !hasDropinsDir && input.dropinsDirListing != null) {
        warnings.push(`${installedDropins.length} drop-in package(s) installed but scripts/__dropins__/ has no storefront-* folders. Run "npm run postinstall".`);
    }
