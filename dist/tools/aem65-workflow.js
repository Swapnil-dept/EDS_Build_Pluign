...
export function registerAem65Workflow(server) {
    server.tool('aem65_workflow', `Route AEM 6.5 LTS / AMS Workflow tasks to the right specialist (workflow-model-design | workflow-development | workflow-triggering | workflow-launchers | workflow-debugging | workflow-triaging | workflow-orchestrator). Mirrors Adobe's "aem-workflow" skill index for 6.5. PRECONDITION: only after detect_project_type returns "aem65lts". Differs from the Cloud Service variant: JMX is allowed on 6.5 (purge / retry / stale detection), Felix SCR is still supported, and design-time models may live under /etc/workflow/models for legacy projects.`, {
        ...
        sections.push(`## 6.5 LTS guardrails for workflow\n\n` +
...
//# sourceMappingURL=aem65-workflow.js.map