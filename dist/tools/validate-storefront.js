...
export function registerValidateStorefront(server) {
    server.tool('validate_storefront', `Validate an EDS commerce storefront project for common issues. Pass the contents of package.json, scripts/initializers.js, scripts/scripts.js, config.json, and head.html. Returns errors (block release), warnings (should fix), and info (suggestions). Checks: drop-in dependency vs postinstall sync, initializer wiring, config completeness, ACDL bootstrap, and pre-LCP discipline.`, {
        ...
        if (packageJson) {
            try {
                ...
                findings.push({ level: 'error', message: `package.json is not valid JSON: ${err.message}` });
            }
        }
        else {
        ...
        if (configJson) {
            try {
                ...
                for (const key of required) {
                    if (!cfg[key])
                        findings.push({ level: 'error', message: `config.json missing required key \`${key}\`.` });
                }
                if (!cfg.headers?.cs) {
                ...
                findings.push({ level: 'error', message: `config.json invalid JSON: ${err.message}` });
            }
        }
        else {
...
//# sourceMappingURL=validate-storefront.js.map