...
export function registerScaffoldStorefrontProject(server) {
    server.tool('scaffold_storefront_project', `Guide for setting up a new AEM Edge Delivery Services + Adobe Commerce storefront project. Returns step-by-step instructions, file layout, drop-in installation lifecycle, configuration templates (default-site.json, default-config.json, demo-config.json, default-query.yaml, head.html), and an initial drop-in selection. Based on hlxsites/aem-boilerplate-commerce.`, {
        ...
        const sections = [];
        sections.push(`# Setting Up "${siteName}" — EDS Commerce Storefront\n\n` +
        ...
        sections.push(`## Step 1 · Clone the boilerplate\n\n` +
            '```bash\n' +
            ...
            '```');
        sections.push(`## Step 2 · Install AEM CLI + dependencies\n\n` +
            '```bash\n' +
            ...
            '```');
        sections.push(`## Step 3 · Install drop-ins\n\n` +
            '```bash\n' +
            ...
            `}\n` +
            '```');
        sections.push(`## Step 4 · Configure backend\n\n` +
...
//# sourceMappingURL=scaffold-storefront-project.js.map