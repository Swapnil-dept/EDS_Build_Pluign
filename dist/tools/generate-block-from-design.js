import { z } from 'zod';
import { generateBlockJS, generateBlockCSS, generateBlockReadme, generateTestHtml, generateSampleContent, } from '../knowledge/block-templates.js';
...
export function registerGenerateBlockFromDesign(server) {
    server.tool('generate_block_from_design', `Generate an AEM EDS block from any combination of text description, image/screenshot, and/or Figma URL. Applies Adobe's Content-Driven-Development workflow (analyze-and-plan → content-modeling → building-blocks → code-review) and emits a scaffold (JS/CSS/README/test.html/sample-content) plus a vision-analysis prompt the IDE LLM should execute against the supplied design inputs.`, {
        ...
        idempotentHint: true,
        openWorldHint: false,
    }, async ({ blockName, text, imageRefs, figmaUrl, figmaToken, variant, layout, hasMedia, interactive, }) => {
        ...
        const sampleContent = generateSampleContent(blockName, [{ name: 'content', type: 'richtext', label: 'Content' }], variant);
...
function buildVisionPrompt(args) {
    ...
    lines.push('2. **Authoring fields** — for each authorable input: name (camelCase), type (text / richtext / image / link / select / boolean / multiselect), required?, example value');
    lines.push('3. **Variants** — CSS class variants visible in the design (dark, wide, centered, …)');
    lines.push('4. **Responsive behavior** — mobile / tablet / desktop layout differences, any breakpoint-specific changes');
    lines.push('5. **Interactivity** — hover, focus, click-to-expand, carousel, etc. (otherwise "none")');
    lines.push('6. **Design tokens** — colors, typography scale, spacing; map to existing CSS custom properties (var(--link-color), var(--heading-font-family), etc.) where possible');
    lines.push('7. **Acceptance criteria** — fill in the Adobe analyze-and-plan template');
    lines.push('');
