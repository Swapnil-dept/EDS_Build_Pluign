import { z } from 'zod';
const TEMPLATES = {
    'default-site': (siteName, locale, currency) => JSON.stringify({
        site: { name: siteName, title: siteName.replace(/-/g, ' ') },
        locale: { code: locale, currency },
        search: { engine: 'live-search' },
    }, null, 2),
    'default-config': (backend, endpoint, envId, storeView, apiKey) => JSON.stringify({
    ...
    'demo-config': (backend) => `// Same shape as default-config.json but pointing to your demo backend.
    ...
    properties:
      ...
      image:
        ...
        value: |
          attribute(el, 'content')
`,
    'default-sitemap': (domain) => `# default-sitemap.yaml
sitemaps:
  default:
    origin: https://${domain}
    ...
    lastmod: YYYY-MM-DD
`,
    'head-html': (endpointOrigin) => `<meta charset="UTF-8" />
...
export function registerEdsStorefrontConfig(server) {
    server.tool('eds_storefront_config', `Generate configuration files for an EDS commerce storefront. Templates: default-site.json, default-config.json, demo-config.json (PaaS/SaaS), demo-config-aco.json (Adobe Commerce Optimizer), default-query.yaml (Helix Query for products), default-sitemap.yaml, head.html (with ACDL bootstrap + preconnect), and scripts/configs.js helper.`, {
        configType: z
        ...
        const out = [];
        const want = (k) => configType === 'all' || configType === k;
        if (want('default-site'))
            out.push({ file: 'default-site.json', lang: 'json', body: TEMPLATES['default-site'](siteName, locale, currency) });
        if (want('default-config'))
            out.push({ file: 'default-config.json', lang: 'json', body: TEMPLATES['default-config'](backend, endpoint, environmentId, storeViewCode, apiKey) });
        if (want('demo-config'))
            out.push({ file: backend === 'aco' ? 'demo-config-aco.json' : 'demo-config.json', lang: 'json', body: TEMPLATES['default-config'](backend, endpoint, environmentId, storeViewCode, apiKey) });
        if (want('default-query'))
            out.push({ file: 'default-query.yaml', lang: 'yaml', body: TEMPLATES['default-query']() });
        if (want('default-sitemap'))
            out.push({ file: 'default-sitemap.yaml', lang: 'yaml', body: TEMPLATES['default-sitemap'](domain) });
        if (want('head-html'))
            out.push({ file: 'head.html', lang: 'html', body: TEMPLATES['head-html'](endpointOrigin) });
        if (want('configs'))
            out.push({ file: 'scripts/configs.js', lang: 'js', body: TEMPLATES.configs() });
        const sections = [`# Storefront config templates (${out.length} file${out.length === 1 ? '' : 's'})`];
        for (const f of out) {
...
//# sourceMappingURL=eds-storefront-config.js.map