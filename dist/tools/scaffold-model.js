...
export function registerScaffoldModel(server) {
    server.tool('scaffold_model', `Generate Universal Editor component model files for an EDS block (aligned with aem-boilerplate-xwalk). Emits entries for component-models.json, component-definition.json, and component-filters.json. Supports container blocks with nested item children (e.g. cards→card, tabs→tab).`, {
        ...
        try {
            ...
            if (!hasClasses && !isContainer) {
                notes.push('**Tip:** add a `multiselect` field named `classes` (with grouped options) to let authors toggle CSS variants on the block (e.g. "dark", "wide"). This is the aem-boilerplate-xwalk convention.');
            }
            if (isContainer) {
                notes.push(`**Container block detected** (${items.length} item type${items.length > 1 ? 's' : ''}). ` +
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Model generation failed: ${error.message}` }],
            };
        }
    });
}
//# sourceMappingURL=scaffold-model.js.map