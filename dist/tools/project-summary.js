import { z } from 'zod';
import { detectProjectType } from '../knowledge/project-detection.js';
function listLines(s) {
    return (s ?? '')
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
...
function extractCssVariables(stylesCss) {
    if (!stylesCss)
        return [];
    return unique([...stylesCss.matchAll(/--([a-z0-9-]+)\s*:/gi)].map((match) => `--${match[1]}`)).slice(0, 12);
}
function extractFontHints(stylesCss, headHtml) {
    const hints = [];
    if (stylesCss) {
    ...
    if (headHtml) {
        if (/fonts\.googleapis/i.test(headHtml))
            hints.push('Google Fonts linked in head.html');
        if (/typekit|adobe-fonts/i.test(headHtml))
            hints.push('Adobe Fonts linked in head.html');
    }
    return unique(hints).slice(0, 6);
}
function extractConfigKeys(configJson) {
    if (!configJson)
        return [];
    try {
...
function summarizeFunctionality(input, projectType) {
    ...
    if (projectType.type === 'storefront') {
        ...
        if (scripts.includes('initializers.js'))
            items.push('Drop-ins are likely mounted through scripts/initializers.js.');
        ...
        if (blocks.length)
            items.push(`EDS block library present: ${blocks.slice(0, 10).join(', ')}`);
        if (root.includes('fstab.yaml'))
            items.push('Content is mounted through fstab.yaml.');
        if (scripts.length)
            items.push(`Global script surface: ${scripts.join(', ')}`);
    }
        ...
        if (root.includes('pom.xml'))
            items.push('Build and packaging are Maven-based.');
        if (listLines(input.dispatcherDirListing).length)
            items.push('Dispatcher configuration is part of the project.');
    }
    if (!items.length) {
...
function summarizeGlobalDefinitions(input) {
    const theme = [];
    const security = [];
    const runtime = [];
    const scripts = listLines(input.scriptsDirListing);
    ...
    if (cssVars.length)
        theme.push(`Theme tokens / CSS variables detected: ${cssVars.join(', ')}`);
    if (fontHints.length)
        theme.push(`Typography hints: ${fontHints.join('; ')}`);
    if (/adobeDataLayer\s*=/.test(input.headHtml ?? ''))
        runtime.push('Adobe Client Data Layer bootstrap detected in head.html.');
    if (/preconnect/i.test(input.headHtml ?? ''))
        runtime.push('head.html includes preconnect hints for global network setup.');
    if (scripts.includes('scripts.js'))
        runtime.push('scripts.js is the global runtime entrypoint.');
    if (scripts.includes('initializers.js'))
        runtime.push('initializers.js handles shared storefront container mounts.');
    if (scripts.includes('configs.js'))
        runtime.push('configs.js centralizes runtime configuration lookups.');
    ...
    if (!theme.length)
        theme.push('No explicit theme CSS input was provided. Pass styles/styles.css or equivalent for better theme extraction.');
    if (!security.length)
        security.push('No explicit auth / encryption / secret-management signal was found in the provided inputs.');
    if (!runtime.length)
        runtime.push('No global runtime files were provided beyond the detection inputs.');
    return { theme, security, runtime };
}
function summarizeSession(existingSummary, sessionChanges) {
    const items = [];
    if (existingSummary)
        items.push('Previous summary was provided and should be treated as the baseline state.');
    if (sessionChanges) {
    ...
    server.tool('project_summary', 'Generate or refresh a workspace summary that captures project type, functional scope, global definitions (theme CSS, runtime scripts, auth/encryption/config signals), and the latest session delta. Use it at the start of a session to create `.project-summary.md` or `PROJECT_SUMMARY.md`, and at the end with `existingSummary` + `sessionChanges` to refresh that file.', {
        ...
        if (!provided) {
            return {
                content: [{
                        type: 'text',
                        text: '# Project summary — no inputs provided\n\n' +
        ...
        if (detection.installedDropins.length)
            lines.push(`- Installed drop-ins: ${detection.installedDropins.join(', ')}`);
        if (detection.detectedAemModules.length)
            lines.push(`- Detected AEM modules: ${detection.detectedAemModules.join(', ')}`);
        lines.push('');
        lines.push('## Functional Scope');
        for (const item of functionality)
            lines.push(`- ${item}`);
        lines.push('');
        lines.push('## Global Definitions');
        lines.push('### Theme / Styling');
        for (const item of globals.theme)
            lines.push(`- ${item}`);
        lines.push('');
        lines.push('### Security / Auth / Encryption');
        for (const item of globals.security)
            lines.push(`- ${item}`);
        lines.push('');
        lines.push('### Runtime / Shared Configuration');
        for (const item of globals.runtime)
            lines.push(`- ${item}`);
        lines.push('');
        ...
        if (detection.recommendedTools.length) {
            ...
            for (const tool of detection.recommendedTools)
                lines.push(`- ${tool}`);
        }
