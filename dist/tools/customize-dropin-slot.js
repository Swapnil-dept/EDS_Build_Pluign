    ...
    server.tool('customize_dropin_slot', `Generate a slot-override snippet for an Adobe Commerce drop-in. Returns a complete decorate() function with the slot wired up using the SlotContext API (appendChild / replaceWith / onChange). Validates the slot exists on the chosen container. Use after add_dropin when you need to inject custom UI (empty state, badges, custom CTAs, trust signals) into a drop-in.`, {
        ...
        if (!slotSpec) {
            return {
                content: [{
                        type: 'text',
                        text: `Slot "${slot}" does not exist on **${cont.name}** in dropin **${spec.id}**.\n\n` +
...
//# sourceMappingURL=customize-dropin-slot.js.map