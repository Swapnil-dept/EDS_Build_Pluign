...
function topN<T>(items: T[], n: number, by: (t: T) => string = String): { value: string; count: number }[] {
...
function findRepeatingClasses(html: string): { className: string; count: number; sampleParent?: string }[] {
  ...
  while ((m = CLASS_RE.exec(html)) !== null) {
    for (const c of m[1].split(/\s+/)) {
      if (c && c.length >= 3 && !/^[0-9]/.test(c)) all.push(c);
    }
  }
  const counts = new Map<string, number>();
...
function detectFramework(html: string): string[] {
  ...
  if (/<meta[^>]+drupal/i.test(html)) hits.push('Drupal');
  if (/cq:template|wcm-content|hlx\.page|aem\.live/i.test(html)) hits.push('AEM / Helix');
  if (/__next_data__|"buildId"/i.test(html)) hits.push('Next.js');
  if (/__nuxt|nuxt-link/i.test(html)) hits.push('Nuxt');
  if (/data-react(?:-helmet|root)/i.test(html)) hits.push('React (SSR)');
  if (/bootstrap(?:\.min)?\.css/i.test(html)) hits.push('Bootstrap CSS');
  if (/tailwind/i.test(html)) hits.push('Tailwind');
  return hits;
}

function detectComponentCandidates(html: string): { type: string; signal: string; count: number }[] {
  const out: { type: string; signal: string; count: number }[] = [];
  const tally = (type: string, signal: string, re: RegExp) => {
    const matches = html.match(re);
    if (matches && matches.length) out.push({ type, signal, count: matches.length });
  };
...
export function registerCrawlUrl(server: McpServer) {
  server.tool(
    ...
    async ({ url, html: providedHtml, userAgent, maxBytes }) => {
      ...
      if (!html) {
        if (!url) {
          return { content: [{ type: 'text' as const, text: 'Either `url` or `html` must be provided.' }], isError: true };
        }
        try {
          ...
          finalUrl = res.url;
          if (!res.ok) {
            return { content: [{ type: 'text' as const, text: `Fetch failed: ${res.status} ${res.statusText} for ${url}` }], isError: true };
          }
          if (!/html|xml/i.test(contentType)) {
            return { content: [{ type: 'text' as const, text: `Refusing to parse non-HTML content-type: ${contentType}` }], isError: true };
          }
          ...
          return { content: [{ type: 'text' as const, text: `Network error: ${e?.message ?? e}` }], isError: true };
        }
      }

      if (!html || html.length < 50) {
        return { content: [{ type: 'text' as const, text: 'No usable HTML content.' }], isError: true };
      }
      ...
      while ((fm = FONT_RE.exec(cssCorpus)) !== null) fontFamilies.push(fm[1].replace(/['"]/g, '').trim());
