...
export function registerEdsConfig(server: McpServer) {
  server.tool(
    ...
    async ({ configType, domain, contentSource, folderId }) => {
      ...
      const shouldInclude = (type: string) => configType === 'all' || configType === type;
      ...
      if (shouldInclude('redirects')) {
        sections.push(
          `## redirects.xlsx — URL Redirect Rules\n` +
      ...
      if (shouldInclude('headers')) {
        sections.push(
          `## headers.xlsx — Custom HTTP Headers\n` +
      ...
      if (shouldInclude('helix-config')) {
        sections.push(
          `## .helix/config.xlsx — CDN & Indexing Configuration\n\n` +
      ...
      if (configType === 'all') {
        sections.unshift(
          `# AEM EDS Project Configuration Guide\n` +
