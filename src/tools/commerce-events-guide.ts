...
export function registerCommerceEventsGuide(server: McpServer) {
  server.tool(
    'commerce_events_guide',
    `Guide for Adobe Client Data Layer (ACDL) and the @dropins/tools event-bus. Returns: how to bootstrap ACDL in head.html, how to install and configure @adobe/magento-storefront-events-sdk + @adobe/magento-storefront-event-collector, the full inventory of dropin events, and snippets for common use cases (cart counter, analytics push, conversion tracking, custom events).`,
    {
      topic: z
    ...
    async ({ topic }) => {
      ...
      const want = (k: string) => topic === 'all' || topic === k;

      if (want('overview')) sections.push(
        `# Adobe Commerce events on EDS\n\n` +
      ...
      if (want('bootstrap')) sections.push(
        `## 1 · Bootstrap ACDL in head.html\n\n` +
        '```html\n' +
      ...
      if (want('collector')) sections.push(
        `## 2 · Install the Storefront Events SDK + Collector\n\n` +
        '```bash\n' +
      ...
      if (want('events')) sections.push(
        `## 3 · Drop-in event inventory\n\n` +
        '| Drop-in | Event | Payload |\n|---|---|---|\n' +
        DROPIN_CATALOG.flatMap((d) =>
          d.events.map((e) => `| ${d.id} | \`${e.name}\` | \`${e.payload}\` |`),
        ).join('\n'),
      );

      if (want('cart-counter')) sections.push(
        `## 4 · Cart counter (mini-cart badge)\n\n` +
        '```js\n' +
      ...
      if (want('analytics')) sections.push(
        `## 5 · Push to analytics (GA4 / Adobe Analytics)\n\n` +
        '```js\n' +
      ...
      if (want('custom-events')) sections.push(
        `## 6 · Emit your own events\n\n` +
        '```js\n' +
        `import { events } from '@dropins/tools/event-bus.js';\n\n` +
      ...
      if (want('all')) sections.push(`## Reference: Event Bus pattern\n\n${EVENT_BUS_PATTERN}`);
