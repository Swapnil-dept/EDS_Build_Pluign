...
export function registerAemSecurityPipeline(server: McpServer) {
  server.tool(
    ...
    async ({ platform = 'both', section = 'overview' }) => {
      ...
      parts.push(
        `# Security Pipeline — ${platform === 'both' ? 'AEM Cloud Service & EDS' : platform === 'aem' ? 'AEM as a Cloud Service' : 'Edge Delivery Services (EDS)'}`,
      );
      ...
      // ── AEM sections ─────────────────────────────────────────────────────
      if (platform !== 'eds') {
        if (want('sast'))       parts.push(PIPELINE_SECTIONS.aem_sast);
        if (want('deps'))       parts.push(PIPELINE_SECTIONS.aem_deps);
        if (want('secrets'))    parts.push(PIPELINE_SECTIONS.aem_secrets);
        if (want('dispatcher')) parts.push(PIPELINE_SECTIONS.aem_dispatcher);
        if (want('checklist'))  parts.push(PIPELINE_SECTIONS.aem_checklist);
      }

      // ── EDS sections ─────────────────────────────────────────────────────
      if (platform !== 'aem') {
        if (want('ci'))         parts.push(PIPELINE_SECTIONS.eds_ci);
        if (want('headers'))    parts.push(PIPELINE_SECTIONS.eds_headers);
        if (want('blocks'))     parts.push(PIPELINE_SECTIONS.eds_blocks);
        if (want('checklist'))  parts.push(PIPELINE_SECTIONS.eds_checklist);
      }
      ...
      if (parts.length <= 1) {
        parts.push(
          `No content matched platform="${platform}" section="${section}".\n\n` +
