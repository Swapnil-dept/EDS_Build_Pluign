...
export function registerScaffoldProject(server: McpServer) {
  server.tool(
    ...
    async ({ projectType, siteName, contentSource, includeUE, includeCommerce }) => {
      ...
      // ─── Step 1: Create Repo ────────────────────────────
      sections.push(
        `# Setting Up "${siteName}" — AEM EDS Project\n\n` +
      ...
      sections.push(
        `## Step 2: Install AEM CLI & Start Dev Server\n\n` +
        `\`\`\`bash\n` +
      ...
      if (projectType !== 'repoless-content') {
        ...
        sections.push(
          `## Step 5: Repoless Content Site Structure\n\n` +
          `\`\`\`\n` +
          `/\n` +
