...
export function registerLookupDropin(server: McpServer) {
  server.tool(
    'lookup_dropin',
    `Look up an Adobe Commerce drop-in. Returns: package name, purpose, all containers (mountable Preact components) with import paths and slots, all ACDL events emitted, the suggested EDS block name, and a quick start mount snippet. Use before customizing a dropin to discover what's available. Pass no query to list every drop-in.`,
    {
      query: z
        .string()
        .optional()
    ...
    async ({ query }) => {
      if (!query) {
        const sections: string[] = [
          ...
          '## B2C',
          DROPIN_CATALOG.filter((d) => d.category === 'b2c').map((d) =>
            `- **${d.id}** — ${d.title} · \`${d.package}\` · block: \`${d.blockName}\``,
          ).join('\n'),
        ];
