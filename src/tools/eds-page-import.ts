...
/**
 * EDS Page-Import skills index — mirrors Adobe's official catalog at
 * https://github.com/adobe/skills/tree/beta/skills/aem/edge-delivery-services/skills
 *
 * The official catalog is the single source of truth. This module surfaces
 * its index, anchors, and one-line descriptions so the IDE LLM can route to
 * the right skill name during a page-migration session.
 */
...
const BLOCK_STRUCTURES: Record<string, { description: string; rows: string; example: string; notes?: string }> = {
  hero: {
...
export function registerEdsPageImport(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ query }) => {
      ...
      if (!matches.length) lines.push('_No skills match._');
      ...
      for (const section of sections) {
        ...
        for (const seq of section.sequences) {
          if (seq.decision === 'default-content') {
            ...
            if (content) inner.push(indent(content, 2));
          } else {
            const block = seq.block?.trim();
            if (!block) {
              return { content: [{ type: 'text' as const, text: 'Sequence with decision=block is missing `block` name.' }], isError: true };
            }
            const rows = seq.rows ?? [];
            if (!rows.length) {
              return { content: [{ type: 'text' as const, text: `Block "${block}" has no rows.` }], isError: true };
            }
