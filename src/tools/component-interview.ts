...
/**
 * `component_interview` — return the canonical question set for collecting
 * authoring fields, variants, and feature toggles before scaffolding a
 * component. One unified shape across EDS, EDS Commerce Storefront,
 * AEMaaCS, and AEM 6.5 LTS / AMS so the IDE LLM can drive the same
 * interview flow regardless of project type.
 *
 * The chat agent is expected to:
 *   1. call `detect_project_type` first
 *   2. call this tool with the resulting type
 *   3. ask the user the questions verbatim
 *   4. call the matching scaffold tool with the collected answers
 */

type ProjectType = 'eds' | 'storefront' | 'aemaacs' | 'aem65lts';
...
interface InterviewSpec {
  ...
  featureToggles: { name: string; question: string; default?: string }[];
  questions: { id: string; question: string; required?: boolean; hint?: string }[];
  jsonTemplate: string;
}

const SPECS: Record<ProjectType, InterviewSpec> = {
  eds: {
    scaffoldTool: 'scaffold_block',
    toolNotes:
      'For Universal Editor authoring use the canonical UE field types. Pair with `scaffold_model` if you need separate component-definitions / models / filters JSON for nested container blocks.',
    fieldTypes: [
