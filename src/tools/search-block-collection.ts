  ...
  server.tool(
    ...
    async ({ query, source }) => {
      ...
      if (source === 'collection' || source === 'both') {
        try {
          const resp = await fetch(
            'https://api.github.com/repos/adobe/aem-block-collection/contents/blocks',
            { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'eds-mcp-server/1.0.0' } }
          );
          if (resp.ok) {
          ...
          errors.push(`Block Collection search failed: ${(e as Error).message}. Check your network connection.`);
        }
      }
      ...
      if (source === 'party' || source === 'both') {
        try {
          const resp = await fetch(
            'https://api.github.com/repos/aem-block-collection/block-party/contents',
            { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'eds-mcp-server/1.0.0' } }
          );
          if (resp.ok) {
            const items = await resp.json() as Array<{ name: string; type: string; html_url: string }>;
          ...
          errors.push(`Block Party search failed: ${(e as Error).message}. Check your network connection.`);
        }
      }
      ...
      if (results.length > 0) {
        ...
        for (const r of results) {
          sections.push(
            `### ${r.name} ${r.match === 'exact' ? '✅ (exact match)' : '🔍 (partial match)'}\n` +
            `**Source:** ${r.source}\n` +
            `**Browse code:** ${r.url}\n` +
        ...
        sections.push(
          `No existing block found for "${query}" in the searched repositories.\n\n` +
          `**Next steps:**\n` +
