...
export function registerStyleDropin(server: McpServer) {
  server.tool(
    'style_dropin',
    `Generate brand-aligned CSS overrides for an Adobe Commerce drop-in using @dropins/tools design tokens (--color-*, --type-*, --spacing-*, --shape-*). Returns a scoped stylesheet that re-themes the dropin without forking it. Pass brand colors, font, and radius — and optionally per-component tweaks.`,
    {
    ...
    async (args) => {
      ...
      const radiusMap: Record<string, string> = { sharp: '2px', soft: '8px', pill: '9999px' };
      const tokens: string[] = [];
      if (brandPrimary) {
        tokens.push(`  --color-brand-500: ${brandPrimary};`);
        tokens.push(`  --color-brand-600: color-mix(in srgb, ${brandPrimary} 88%, black);`);
      }
      if (brandPrimaryDark) tokens.push(`  --color-brand-700: ${brandPrimaryDark};`);
      if (neutralBase)      tokens.push(`  --color-neutral-50: ${neutralBase};`);
      if (headingFont)      tokens.push(`  --type-headline-font-family: ${headingFont};`);
      if (bodyFont)         tokens.push(`  --type-base-font-family: ${bodyFont};`);
      if (radius)           tokens.push(`  --shape-radius-2: ${radiusMap[radius]};\n  --shape-radius-3: ${radiusMap[radius]};`);
