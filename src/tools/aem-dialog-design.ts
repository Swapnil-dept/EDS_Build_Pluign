...
import {
  ...
  DIALOG_PATTERNS,
  NAMING_CONVENTIONS,
} from '../knowledge/aem-dialog-design.js';

const DIALOG_DESIGN_GUIDE_ROOT =
  ...
  server.tool(
    ...
    async ({ query }) => {
      ...
      if (!q) {
        ...
        sections.push(
          `## Core Principles (${DIALOG_PRINCIPLES.length})\n\n` +
            DIALOG_PRINCIPLES.map(
              (p) =>
                `- **${p.id}** — ${p.title}: ${p.description.substring(0, 80)}...`,
            ).join('\n') +
            `\n\nUse a principle keyword to dive deeper, e.g., **naming-consistency**, **coralui-compliance**, **tooltips-and-help**.`,
        );

        sections.push(
          `## Field Types (${DIALOG_FIELD_TYPES.length})\n\n` +
        ...
        sections.push(
          `## Dialog Patterns (${DIALOG_PATTERNS.length})\n\n` +
      ...
      // ─── Search: Principles ─────────────────────────────────────────
      const matchedPrinciples = DIALOG_PRINCIPLES.filter(
        (p) => p.id.includes(q) || p.title.toLowerCase().includes(q),
      );

      if (matchedPrinciples.length > 0) {
        const sections: string[] = [];
        sections.push(
          `# AEM Dialog Design — Principles\n\n` +
      ...
      // ─── Search: Field Types ────────────────────────────────────────
      const matchedFields = DIALOG_FIELD_TYPES.filter(
        (f) =>
          f.id.includes(q) ||
      ...
      if (q.includes('naming')) {
        ...
        sections.push(
          `## Component Sorting Codes\n\n` +
            ...
            Object.entries(NAMING_CONVENTIONS.codePrefix)
              .map(
                ([code, category]) =>
                  `| \`${code}\` | ${category.split(' ')[0]} | ${category.substring(category.indexOf('('))} |`,
              )
              .join('\n'),
        );
        ...
        sections.push(
          `## Field Naming Pattern (Substance First)\n\n` +
        ...
        sections.push(
          `## Property Name Convention (camelCase)\n\n` +
      ...
      if (q.includes('pattern') || q.includes('grid') || q.includes('hero') || q.includes('card')) {
        const matchedPatterns = DIALOG_PATTERNS.filter(
          (p) =>
