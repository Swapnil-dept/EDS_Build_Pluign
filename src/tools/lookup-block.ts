...
export function registerLookupBlock(server: McpServer) {
  server.tool(
    ...
    async ({ query }) => {
      ...
      const matches: Array<{ name: string; score: number; pattern: typeof BLOCK_PATTERNS[string] }> = [];
      ...
      for (const [name, pattern] of Object.entries(BLOCK_PATTERNS)) {
        ...
        // Partial name match
        if (name.includes(searchTerm) || searchTerm.includes(name)) score += 50;
        // Description match
        if (pattern.description.toLowerCase().includes(searchTerm)) score += 30;
        // Field name match
        if (pattern.fields.some((f) => f.name.toLowerCase().includes(searchTerm))) score += 20;
        // Keyword matches
      ...
      if (matches.length > 0) {
        sections.push(
          `## Found ${matches.length} matching block pattern(s) for "${query}"\n`
        );
        ...
        sections.push(
          `No built-in pattern found for "${query}". This is a custom block — here's how to approach it:\n\n` +
