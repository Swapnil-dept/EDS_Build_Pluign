...
export const AEM_65_LTS_SKILLS: Aem65LtsSkill[] = [
  {
    id: 'ensure-agents-md',
    title: 'Ensure AGENTS.md & CLAUDE.md (6.5 LTS variant)',
    description:
      'Bootstrap step. Run FIRST in any AEM 6.5 LTS / AMS workspace that lacks `AGENTS.md`. Detects modules and add-ons from `pom.xml`, generates an AGENTS.md tailored for AEM 6.5 (not Cloud Service), and creates `CLAUDE.md` (`@AGENTS.md`). Resource links point to `experienceleague.adobe.com/.../experience-manager-65/...`, never to Cloud Service URLs.',
    when:
      'Workspace root has no AGENTS.md and `pom.xml` references `com.adobe.aem:uber-jar` (any classifier), `cq-quickstart`, or `cq.quickstart.version` / `aem.version` in the 6.5 line. Excludes Cloud Service projects (those use the cloud-service variant).',
  ...
  },
  {
    ...
    description:
      'Single entry point for AEM 6.5 LTS Replication. Covers configuring agents (default, dispatcher flush, reverse), activating/deactivating content (Quick Publish / Manage Publication / Tree Activation / Package Manager / Workflows / Scheduled), the Replication API (Replicator, ReplicationOptions, ReplicationStatus, AgentManager, ReplicationQueue, ReplicationListener), and troubleshooting (blocked queues, connection errors, dispatcher cache, agent config).',
    when:
      'Replication agent setup, content activation/deactivation, Replicator API integration, blocked queues, connectivity issues, distribution problems. NOT applicable to Cloud Service (which uses Sling Distribution).',
  ...
  },
  {
    ...
    description:
      'Single entry point for AEM 6.5 LTS Granite Workflow. Routes to specialists for model design, custom WorkflowProcess / ParticipantStepChooser development, launchers, triggering (incl. replication triggers), debugging stuck/failed workflows, triaging with JMX + Splunk + direct log access, thread pool analysis, and Sling Jobs.',
    when:
      'Workflow models, WorkflowProcess, ParticipantStepChooser, launchers, Inbox tasks, stuck/failed workflows, JMX-driven retry/purge, replication-triggered workflows.',
  ...
  },
  {
    ...
    description:
      'Single entry point for the Apache HTTPD Dispatcher module on AEM 6.5 LTS / AMS. Routes to specialists for config authoring, technical advisory, incident response, performance tuning, security hardening, and full-lifecycle orchestration. Dispatcher MCP variant: `AMS` (`AEM_DEPLOYMENT_MODE=ams`).',
    when:
      'Editing `.any` / vhost / rewrite / cache / filter files; debugging cache anomalies; tuning latency/throughput; auditing exposure / headers on AMS or on-prem.',
...
export function findAem65LtsSkill(query: string): Aem65LtsSkill | undefined {
  const q = query.toLowerCase().trim();
  return AEM_65_LTS_SKILLS.find(
    (s) => s.id === q || s.title.toLowerCase() === q || s.id.includes(q) || s.title.toLowerCase().includes(q),
  );
}
