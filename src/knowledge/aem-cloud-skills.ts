...
export interface AemCloudSkill {
  ...
  subSkills?: { id: string; description: string }[];
}

const ROOT = 'https://github.com/adobe/skills/tree/beta/skills/aem/cloud-service/skills';

export const AEM_CLOUD_SKILLS: AemCloudSkill[] = [
  {
    id: 'ensure-agents-md',
    title: 'Ensure AGENTS.md & CLAUDE.md',
    description:
      'Bootstrap step. Run FIRST in any AEMaaCS workspace that lacks `AGENTS.md` at the repo root. Detects modules and add-ons from `pom.xml`, generates a tailored AGENTS.md, and creates `CLAUDE.md` (`@AGENTS.md`).',
    when:
      'Workspace root has no AGENTS.md and `pom.xml` references `aem-sdk-api` / `aem-project-archetype` or has classic AEM modules (core, ui.apps, dispatcher).',
  ...
  },
  {
    ...
    description:
      'Holds the **pattern reference modules** for Cloud Service-correct Java/OSGi code and HTL lint fixes (scheduler, ResourceChangeListener, replication, JCR observation event listener, OSGi Event Admin event handler, DAM AssetManager, Felix SCR → OSGi DS, ResourceResolver/SLF4J, HTL `data-sly-test` redundant constant).',
    when:
      'Greenfield/maintenance Java work, refactoring legacy patterns, or fixing HTL Cloud SDK lint warnings without BPA orchestration.',
  ...
  },
  {
    ...
    description:
      'Creates complete AEM components: `.content.xml`, `_cq_dialog/`, HTL template, Sling Model + tests, clientlibs, optional Sling Servlet. Supports extending Core Components via `@Self @Via(type = ResourceSuperType.class)` delegation. Reads `.aem-skills-config.yaml` (project root) before doing anything.',
    when:
      'User asks to create / build / scaffold an AEM component (teaser, card, hero, banner, accordion, tabs, carousel, list, navigation, breadcrumb, custom). Extending Core Components, adding dialogs, building Sling Models, or converting Figma to HTL.',
  ...
  },
  {
    ...
    title: 'Dispatcher',
    description:
      'Single entry point for the Apache HTTPD Dispatcher module on AEMaaCS. Routes to specialists for config authoring, technical advisory, incident response, performance tuning, security hardening, and full-lifecycle orchestration.',
    when:
      'Editing `.any` / vhost / rewrite / cache / filter files; debugging probe regressions or cache anomalies; tuning latency/throughput; auditing exposure / headers.',
  ...
  },
  {
    ...
    description:
      'Orchestrates legacy AEM 6.x / AMS / on-prem → AEMaaCS migration. Drives off **BPA** (CSV / cached collection) or **CAM via MCP**. Enforces **one pattern per session**. Delegates transformation to the `best-practices` references.',
    when:
      'User has BPA/CAM findings, Cloud Service blockers, or wants to migrate code paths like scheduler, ResourceChangeListener, replication, EventListener, OSGi EventHandler, DAM AssetManager, or HTL `data-sly-test` lint.',
    path: `${ROOT}/migration`,
    status: 'beta',
  },
  {
    ...
    title: 'AEM Workflow',
    description:
      'Single entry point for AEMaaCS Granite Workflow. Routes to specialists for model design, custom process step / participant chooser development, launcher configuration, workflow triggering, debugging stuck/failed workflows, triaging with Cloud Manager logs, thread pool analysis, and Sling Job diagnostics.',
    when:
      'User mentions workflow models, WorkflowProcess, ParticipantStepChooser, launchers, Inbox tasks, stuck/failed workflows, thread pool exhaustion, or workflow purge.',
...
export function findAemCloudSkill(query: string): AemCloudSkill | undefined {
  const q = query.toLowerCase().trim();
  return AEM_CLOUD_SKILLS.find(
    (s) =>
      s.id === q ||
...
// ─── Pattern map for migration / best-practices ─────────────────

export interface PatternRef {
  id: string;             // BPA pattern id (or — for HTL lint)
  title: string;
  module: string;         // references/<file>.md
  classification: string; // imports / types that signal this pattern
}

export const AEM_CLOUD_PATTERNS: PatternRef[] = [
  {
    id: 'scheduler',
    title: 'Scheduler',
...
export const AEM_DIALOG_FIELD_MAP: Array<{ user: string; resourceType: string; note?: string }> = [
...
export const CORE_COMPONENT_MAP: Record<string, string> = {
