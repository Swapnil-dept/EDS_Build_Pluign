...
import { type BlockFiles, type ValidationResult, validateBlock } from './validator.js';
...
function looksLikeBlockDir(dir: string): boolean {
  const name = dir.split('/').filter(Boolean).pop() ?? '';
  return existsSync(join(dir, `${name}.js`)) || existsSync(join(dir, `${name}.css`));
}

function readSafe(dir: string, file: string): string | undefined {
  const target = join(dir, file);
  return existsSync(target) ? readFileSync(target, 'utf8') : undefined;
}

function validateDirectory(dir: string): ValidationResult {
  const name = dir.split('/').filter(Boolean).pop() ?? '';
...
async function run(): Promise<void> {
  ...
  console.log(
    `${results.length} block(s) validated   ` +
